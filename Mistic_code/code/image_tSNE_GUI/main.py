...
def draw_tSNE_scatter(tsne1, file_name_hover,cluster_ms_list ):
    ...
    #2024: legend to legend_label; plot_width to width, plot_height to height
    p1 = figure(width=400, height=400, tooltips=TOOLTIPS,tools = TOOLS,
               title="Patient response")

    p1.scatter('x', 'y', size=10, source=source, legend_label= 'res_list', color = 'color_vec_list',fill_alpha=0.6)
    p1.legend.location = "bottom_left"


    p2 = figure(width=400, height=400, tooltips=TOOLTIPS,tools = TOOLS,
               title="Treatment category")
    p2.scatter('x', 'y', size=10, source=source, legend_label= 'tx_list', color = 'color_vec_tx_list',fill_alpha=0.6)
    p2.legend.location = "bottom_left"
    

    p3 = figure(width=400, height=400, tooltips=TOOLTIPS,tools = TOOLS,
               title="Cluster annotations")
    p3.scatter('x', 'y', size=10, source=source, legend_label= 'cluster_anno_list', color = 'color_vec_clasgn_list',fill_alpha=0.6)
    ...
    p4 = figure(width=400, height=400, tooltips=TOOLTIPS,tools = TOOLS,
               title="Patient id")
    p4.scatter('x', 'y', size=10, source=source, legend_label= 'cluster_pat_list', color = 'color_vec_patid_list',fill_alpha=0.6)
...
def generate_stack_montage(chk_box_marker_sm, rb_imtech_val, LABELS_MARKERS):
    ...
    m = np.int64(np.floor(nx*ny/num_images)) #2024 np.int to int64
    ...
    for k in range(len(inds)):
        ...
        for m in range(1):
            ...
            bw = closing(image > thresh)# 2024, square(1))
        ...
        tl = sum(image_all_2) #2024

        tl=tl[0,0:tl.shape[1],0:tl.shape[2]] #2024
...
def generate_image_tSNE(chk_box_marker,rb_val,rb_rs_val,rb_shf_val, rb_imtech_val, mc, wc, LABELS_MARKERS):
    ...
    if (rb_shf_val == 0): # no shuffle option
        ...
        for k in range(len(inds)):
            ...
            for m in range(len(marker_choice)):
                ...
                print('############marker_choice:',str (marker_choice)) #2024
                ...
                bw = closing(image > thresh)#, square(1)) #2024

                # remove artifacts connected to image border
                cleared_imtsne = clear_border(bw)


                image_all_1.append(cleared_imtsne *weight_choice[m]) #2024
            ...
            if (rb_imtech_val ==0): #vectra
                tile_1 = Image.fromarray(np.uint8(cm.viridis(tl)*255)) #2024 commented this added below
                #tile_1 = Image.fromarray(np.uint8((tl)*255)) #2024 commented this added below
                #tile_1 = Image.fromarray((tl).astype(np.uint8),'RGB')
    ...
    ##code for reshuffling
        ...
        for k in range(N): 
            ...
            count_file = np.int64(np.array(np.where(shf_N[k]==shf_N)).flatten()) #2024 np.int to int64
...
def button_callback():
    ...
    tab4 = TabPanel(child=p4, title="Patient id") 
    ...
    if (stack_montage_flag==True):
        if (rb_imtech_val ==1): #SM for t-CyCIF data
            ...
            for i in range(markers_single.shape[0]):
                ...
                channel_num = np.int64(np.array(np.where(channel_name==markers_single)).flatten()) #2024 np.int64
...
if os.path.isfile(fname):
    ...
    # perform tSNE
    tsne = TSNE(n_components=2, verbose=1, perplexity=5)#, n_iter=900) #2024, setting perplexity value to be <n_samples
...
if os.path.isfile(fname):
    ...
    for i in range(resp_list_1.shape[0]):
        row_t = np.int64(np.array(np.where(resp_list_1[i]==uni_resp)).flatten()) #2024, int to int64 
        '''
        print('###########row_t')
        print(type(row_t))
        print(row_t)
        row_t.reshape((1, -1))
        print(row_t)
        row_t = np.int64(row_t)
        print(type(row_t))
        print(row_t.item()
        '''
        color_vec.append(colours_resp[row_t.item()])
...
if os.path.isfile(fname):
    ...
    for i in range(tx_list_1.shape[0]):
        row_t = np.int64(np.array(np.where(tx_list_1[i]==uni_tx)).flatten()) #2024, int to int64
     
        color_vec_tx.append(colours_tx[row_t.item()])
...
if os.path.isfile(fname):
    ...
    for fname1 in os.listdir(FoV_path):
        ...
        for m in range(num_markers):
                ...
                bw = closing(image1 > thresh)#2024, square(1))
...
#2024 plot_width to width, same for height
...
point_tSNE.scatter(tsne[:,0],tsne[:,1],fill_alpha=0.6, color ='red',size=8,legend_label='Response') #2024 legend to legend_label

point_tSNE.legend.location = "bottom_left"

#2024 Figure to figure
...
p1 = figure(width=400, height=400, tooltips=TOOLTIPS,tools = TOOLS,
               title="Patient response") #2024 width and height
